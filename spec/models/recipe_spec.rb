...
RSpec.describe Recipe, type: :model do
  ...
  it 'is valid with a name, preparation_time, cooking_time, and a user' do
    recipe = Recipe.new(name: 'Delicious Dish', preparation_time: 30, cooking_time: 60, user: user, description: 'test description')
    expect(recipe).to be_valid
  end

  it 'is not valid without a name' do
    recipe = Recipe.new(name: nil, preparation_time: 30, cooking_time: 60)
    expect(recipe).to_not be_valid
  end
  it 'is not valid without preparation_time' do
    recipe = Recipe.new(name: 'Delicious Dish', preparation_time: nil, cooking_time: 60)
    expect(recipe).to_not be_valid
  end
  it 'is not valid without cooking_time' do
    recipe = Recipe.new(name: 'Delicious Dish', preparation_time: 30, cooking_time: nil)
    expect(recipe).to_not be_valid
  end
  ...
  it 'can have associated recipe foods' do
    recipe = Recipe.new(name: 'Delicious Dish', preparation_time: 30, cooking_time: 60)
