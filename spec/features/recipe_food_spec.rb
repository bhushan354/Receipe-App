...
RSpec.describe 'RecipeFood', type: :feature do
  before(:each) do
    ...
    @recipe = Recipe.create(name: 'Special Pizza', preparation_time: 30, cooking_time: 20, description: 'The best',
  ...
  describe 'RecipeFood validation on index page' do
    it 'displays Remove button for each recipe food' do
      visit recipes_path
      expect(page).to have_content('Remove')
    end

    it 'displays Edit button for each recipe food' do
      visit recipes_path
    ...
    it 'displays recipe name (additional check)' do
      visit recipes_path
      expect(page).to have_content('Special Pizza')
    end

    it 'indicates if a recipe is public (additional check)' do
      visit recipes_path
      expect(page).to have_content('Public')
    end
  end
